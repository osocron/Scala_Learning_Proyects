...
object CsvParser {
  /**
    ...
    */
  def parse(source: CsvSource): Array[Array[String]] = parse()(source)
  ...
  /**
    ...
    * @param source The csv data source to operate on
    */
  def parse(cols: Seq[Int] = List(), params: CsvParams = CsvParams())(source: CsvSource): Array[Array[String]] = {
    ...
    if (locs.length == 0) locs = firstLine.indices.toArray
    ...
    val fields = for {
      (x, i) <- firstLine.zipWithIndex
      y <- locs
      if i == y
    } yield x
    fields.toSeq.zipWithIndex.foreach { case (s, i) => addToBuffer(s, i) }
